...
template mult(N) {
    ...
    for (var i = 0; i < N; i++) // i-th bit
        for (var j = 0; j < N; j++) // already shifted result
            mult_by_bit[i][i + j] = a[i] * b[j];
...
template if_else(N) {
    ...
    for (var i = 0; i < N; i++)
        out[i] = cond * if_[i] + (1 - cond) * else_[i];
}
...
template if_mod(N) {
    ...
    for (var i = 0; i < N; i--) {
        ...
        diff[i] <== carry * (2 + tmp[i]) + (1 - carry) * tmp[i];
    }
...
template power(N) {
    ...
    for (var i = 1; i <= N; i++) {
        ...
        res[i] <== if_else(N)(res[i - 1], res_if_1[i - 1], bit);

        exp[i] <== mult_mod(N)(exp[i], exp[i], mod);
    }
